...
import { ringData } from '../utilis/objectData/ringData';
...
function Rings() {
  ...
  const [filteredData, setFilteredData] = useState(ringData);
  const mainContentRef = useRef(null);
  const sidebarRef = useRef(null);

  // Filter and sort logic
  useEffect(() => {
    let filtered = [...ringData];
  ...
  }, [selectedType, priceMax, sortType]);
    ...
    <div>
      ...
      <div className='flex justify-center mb-[50px] relative'>
        ...
        <div
          ...
          style={{ filter: sidebarOpen ? 'blur(3px)' : 'none' }}
        >
          ...
          <div className='flex items-center justify-between flex-wrap'>
            ...
            <p className='text-[14px] text-white mt-2 text-left mr-[620px] mb-2'>
              Showing 1-{filteredData.length} of {ringData.length} results
            </p>
              ...
              {popularity && (
                <div className='absolute right-0 mt-2 w-48 bg-gray-800 shadow-md z-20 border border-white rounded-md'>
                  <div className='flex flex-col'>
                    <button onClick={() => setSortType('popularity')} className='p-2 hover:bg-gray-700 text-left text-[12px]'>Sort by Popularity</button>
                    <button onClick={() => setSortType('rating')} className='p-2 hover:bg-gray-700 text-left text-[12px]'>Sort by Average Rating</button>
                    <button onClick={() => setSortType('latest')} className='p-2 hover:bg-gray-700 text-left text-[12px]'>Sort by Latest</button>
                    <button onClick={() => setSortType('lowToHigh')} className='p-2 hover:bg-gray-700 text-left text-[12px]'>Sort by Price: Low to High</button>
                    <button onClick={() => setSortType('highToLow')} className='p-2 hover:bg-gray-700 text-left text-[12px]'>Sort by Price: High to Low</button>
                  </div>
                </div>
              )}
            </div>
